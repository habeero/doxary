...
Opening a document from the Documents tab uses `/documents/:clientDocumentId` and reads the latest persisted `DocumentAnalysis` from Drift. The result view renders available structured sections and explicitly labels complete, partial, and unavailable outcomes; it never re-requests the backend just to display a local result.
