...
void main() {
  ...
  test(
    'Timed Task schedules at local due time minus its reminder lead',
    () async {
  ...
  test(
    'Only null is no reminder; zero remains an active timed reminder',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
  ...
  test(
    'Removing a reminder, completing, and deleting cancel delivery',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
      ...
      expect(
        await reconciler.reconcile(
          _task(status: TaskStatus.completed, reminderMinutesBefore: 0),
        ),
  ...
  test(
    'Reopening only schedules a reminder whose local instant is future',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
  ...
  test(
    'All Day reminder at due time uses the 09:00 local MVP anchor',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
      final allDay = _task(
  ...
  test(
    'All Day reminder lead time is applied to the 09:00 local anchor',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
  ...
  test(
    'Past All Day reminder remains unscheduled and preserves intent',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
      final allDay = _task(
  ...
  test(
    'Timed and All Day transitions replace the same Task reminder',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
  ...
  test(
    'All Day save, edits, removal, completion, reopen, and delete reconcile',
    () async {
      final scheduler = _FakeScheduler();
      final reconciler = TaskReminderReconciler(scheduler, now: () => now);
      final initial = _task(
      ...
      expect(
        await reconciler.reconcile(
          _task(allDay: true, reminderMinutesBefore: null),
        ),
        TaskReminderOutcome.noReminder,
      );
      expect(
  ...
  test(
    'Permission and platform failures are typed and do not escape',
    () async {
      final denied = _FakeScheduler(
        result: ReminderScheduleResult.permissionDenied,
      );
      final failed = _FakeScheduler(
        result: ReminderScheduleResult.platformFailure,
      );
      final task = _task(
        dueAt: DateTime(2026, 9, 22),
        dueTimeMinutes: 9 * 60,
