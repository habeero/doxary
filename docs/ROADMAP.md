...
The current Settings implementation scope is closed for capabilities whose product and technical semantics are decided: Language, Appearance, About Version/Share, Task reminder preference, and the targeted Home dark-theme compatibility fix. These are physically validated. The Analysis Processing dialog theme fix is implemented with focused widget coverage, but physical visual validation remains. This does not make Settings release-complete. Analysis-completed notifications, Privacy & Data, Legal, Account/Plan, store-rating activation, and the conditional third-language localization review remain tracked by their dependencies in [UI interactions](ui-ux/INTERACTIONS.md#settings-and-notifications). The next post-Settings UI/UX follow-up is the global navigation/back-stack audit in [Navigation](ui-ux/NAVIGATION.md#deferred-navigation-and-back-stack-audit); the broader development order below is unchanged, with Application behavior / remaining MVP gaps still the current root phase.
...
1. **Current — Application behavior / remaining MVP gaps:** complete other explicitly approved application behavior gaps. Original-source access/opening and local Task reminder scheduling/delivery are implemented; source ownership, retention, and deletion distinctions remain subject to documented decisions.
...
3. **Closed for current implementation scope — Pre-release Settings capability completion:** currently decided Language, Appearance, About Version/Share, and Task reminder preference capabilities are implemented, with relevant theme fixes. Dependency-bound Settings items remain tracked below; this closure is not release completion.
...
- **Current application behavior gaps:** original-source access/opening and local Task reminder scheduling/delivery are implemented. Source ownership, retention, and deletion distinctions—including Analysis versus Document versus locally owned source copies—remain subject to documented decisions. See [source lifecycle](app-logic/APPLICATION_LOGIC.md#source-document-access-deletion-and-future-synchronization) and [local Task reminders](app-logic/APPLICATION_LOGIC.md#local-task-reminder-scheduling).
...
- **Settings dependency-tracked work:** Analysis-completed notifications await an MVP decision; Privacy & Data awaits local lifecycle semantics; Legal awaits approved public pages; Account/Plan awaits future account/server and monetization decisions; Rate awaits a real store listing; and localization architecture review is required before adding a third UI language. The current decided implementation scope is closed, but these dependencies keep Settings from being release-complete. See [UI interactions](ui-ux/INTERACTIONS.md#settings-and-notifications).
