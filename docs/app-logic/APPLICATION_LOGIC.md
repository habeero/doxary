...
## Source-document management, deletion, and future synchronization

Doxary has not yet implemented a complete user-facing lifecycle for original locally stored or imported source documents. Document metadata and a physical/local source file are distinct: a Document can retain its metadata and analysis history independently from a locally owned source copy. Future work must deliberately cover showing and opening the original imported file, retention of locally owned source files, Document-record deletion, independent analysis-result deletion, and optional removal of Doxary's local source copy.
