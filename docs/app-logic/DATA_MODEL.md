...
| AnalysisAttempt | Stable attempt ID, Document identity, started/terminal timestamps, safe status/failure metadata, optional result ID, result status, and deletion timestamp | Pending, succeeded, or failed; deletion metadata may mark a previously persisted result as deleted without retaining its payload. Separate from transient polling correlation. |
...
Local schema evolution is explicit and forward-only; schemas are never reset or treated as disposable. Analysis-attempt-history timestamps are stored as Unix milliseconds and read as Dart `DateTime` values in the device-local time path. The v10 migration repairs existing seconds-scale attempt timestamps while preserving millisecond values and null terminal times. The v11 migration adds nullable deletion time and prior result-status metadata to attempt history; it preserves existing rows and does not infer historical deletions. The analysis-attempt-history migration backfills existing successful analysis rows and terminal operation outcomes without changing Document, Analysis, Task, or classification IDs; Task source-provenance columns are nullable for existing/manual Tasks. Pending analysis metadata is client-owned and only correlates work to an existing Document. Fresh analysis versions persist the mapped `action_required` enum token (`yes`, `no`, or `uncertain`) exactly; the forward migration leaves pre-existing rows null rather than inferring action from facts or text. Persisting a new analysis version must not alter user-confirmed Organization or Case relationships.

Completed analyses persist their supported mapped Result data locally: document date, detected language, urgency, confidence, practical states, root uncertainties, next actions, quality reasons, source references, and typed extracted deadlines, appointments, amounts, required documents, and suggested tasks. These analysis-owned child records are keyed by analysis version and remain distinct from document-level operational facts and user-confirmed Tasks. Re-saving the same analysis version replaces its analysis-owned children atomically; legacy rows without those records restore as empty/null data without inference. Attempt history retains safe success/failure outcomes without duplicating result payloads. Deleting an Analysis physically removes its result row and analysis-owned children while preserving a minimal Document-scoped history tombstone with deletion time and the prior persisted result status. The tombstone adds no extracted or user content. Deletions made before this metadata existed cannot be reconstructed. Tasks retain non-cascading provenance and Documents/DocumentFiles remain unchanged.
