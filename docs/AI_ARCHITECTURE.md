...
For MVP questions and reply drafts, the backend is stateless with respect to local documents. The client submits a minimum, operation-scoped envelope containing `client_document_id`, the validated structured analysis when useful, and only the required source evidence/text and conversation context. The client ID correlates the result to its local Document; it is not a server lookup key. Request context is temporary processing data and does not require accounts, cloud sync, or permanent original-file storage. A future synchronized `server_resource_id` may be accepted by a separate compatible route without replacing this local-first operation.
...
5. Generate explanation in requested target language from validated data and source evidence. Delete temporary uploads and request context according to the retention policy.
