...
**Current implementation status:** Camera Capture and single-photo Review are implemented focused stages. Capture shows a real preview, a Back affordance, shutter, and flash only when supported. Permission denied, settings-required/restricted, unavailable, and initialization-failure states use localized user-safe messaging. Shutter pauses the camera and shows the captured file in Review. Retake discards the temporary candidate and safely resumes Capture; Use photo promotes it to the existing Analyze session draft without creating a Document or operation. Review provides real 90-degree image rotation; Crop remains deferred rather than appearing as a non-functional control. Multi-page Review and the final polished camera-selected Analyze state remain upcoming.
