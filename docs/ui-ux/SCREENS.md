...
**Current implementation status:** Processing uses a `Dialog` whose surface, stage surface, primary/secondary text, and neutral stage icon colors come from the active Material `ColorScheme`. Its semantic primary stage emphasis and existing layout/actions remain unchanged. It therefore follows System, Light, and Dark themes without a feature-specific dark palette; focused widget coverage checks Light/Dark, German/Arabic, and narrow-width rendering. Physical-device visual validation is not recorded here and remains part of release review.
...
**Deferred information architecture gap:** Failed/deleted Analysis activity should not permanently occupy normal Documents browsing, which should remain focused on usable/current Documents. The current collection is still the implemented state. During the Settings/information-architecture review, decide a more appropriate dedicated location for past/failed/deleted Analysis activity; do not choose or implement a destination yet. The current “Documents need attention” terminology and its count grouping also need product review because one Document may have multiple Analysis events; see [Interactions](INTERACTIONS.md#home-attention).
...
**Current implementation status:** Settings is implemented as a primary bottom-navigation destination with the approved Account, Language, Notifications, Appearance, Privacy & Data, Legal, and About hierarchy. App language, Explanation language, and Appearance show human-readable values and change through focused selection sheets. Appearance supports System, Light, and Dark, defaults to System without a valid saved choice, persists locally, and updates immediately; Settings surfaces follow the active theme. Explanation language reuses `analysisLanguageProvider` and is observed by future Analyze and Document retry flows; changing it does not change UI language. About shows runtime version/build information and offers native sharing; Rate is intentionally disabled until a canonical store listing exists. The Tasks and appointments notification switch is functional. Analysis-completed notifications remain unavailable and deferred; their MVP inclusion must be decided before release and is not yet a required MVP capability. Account/plan, Legal, and Privacy & Data rows remain visible but unavailable for different reasons; see [Interactions](INTERACTIONS.md#settings-and-notifications) for their distinct deferrals. The active implementation phase is closed for capabilities with decided semantics, not release-complete; validation and deferred dependencies are recorded in Interactions.
...
The Settings implementation phase closed without adding a discoverable entry for historical Analysis activity. Whether to expose “Analysis History,” “Analysis Activity,” or “Document / Analysis History,” and where it should live, remains undecided alongside failed/deleted Analysis lifecycle semantics. Do not add a Settings tile or shortcut, or select a destination yet; possible homes include a future Settings entry or another dedicated management surface. Keep “Local documents” and “Manage data” unavailable until local lifecycle behavior is defined. The global navigation/back-stack audit is the next UI/UX follow-up; it may revisit overall navigation information architecture but does not reverse the current no-sidebar decision. See [Navigation](NAVIGATION.md#deferred-navigation-and-back-stack-audit).

Settings is a primary destination. Its root hierarchy is Account (Profile, plan/account), Language (App language and Explanation language), Notifications, Appearance, Privacy and data, Legal, and About. Notification behavior separates OS permission from Doxary preferences. The Tasks and appointments row is a Doxary-level switch, default ON and persisted under `task_reminders_enabled`; OFF cancels scheduled Task notifications while preserving Task reminder intent, and ON reconciles eligible future open Tasks without prompting for OS permission. A localized read-only permission status is shown when available. Analysis-completed notifications remain deferred and unavailable; category controls are not implemented. Timed local Task delivery and Task notification taps to Task Detail remain unchanged.
